...
async function tryCreateTableViaPg(): Promise<{ ok: boolean; error?: string }> {
  for (const config of DB_CONFIGS) {
    let client: InstanceType<typeof Client> | null = null;
    try {
      client = new Client({ connectionString: config.connectionString, ssl: { rejectUnauthorized: false }, connectionTimeoutMillis: 10000 });
  ...
  return { ok: false, error: 'Cannot reach database directly. Apply migrations manually in Supabase SQL editor.' };
}
...
export default async function handler(req: any, res: any) {
  ...
  if (!SUPABASE_URL || !SERVICE_ROLE_KEY) {
    return res.status(503).json({ error: 'Server not configured (missing SUPABASE_SERVICE_ROLE_KEY)' });
  }
  ...
  if (req.method === 'GET') {
    ...
    if (error) {
      if (isTableMissingError(error)) {
        return res.status(200).json({ campaigns: [], tableExists: false });
      }
  ...
  // POST — create campaign (auto-creates table if missing)
  if (req.method === 'POST') {
    const body = req.body || {};
    const payload = {
      title: body.title,
    ...
    if (error) {
      if (isTableMissingError(error)) {
        // Table missing — try to create it via pg then retry insert
        ...
        const { data: data2, error: error2 } = await admin.from('campaigns').insert(payload).select().single();
