...
export default function AuthPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      if (msg.includes('Email not confirmed')) {
        ...
        setError('An account with this email exists. Try signing in instead.');
        setMode('signin');
      } else {
        setError(msg);
      }
  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 80px)', background: 'var(--bg)', padding: '40px 16px 80px', display: 'flex', alignItems: 'center' }}>
      <div className="mx-auto grid max-w-6xl gap-8 w-full lg:grid-cols-[1.1fr_0.9fr] items-center">

        {/* Left: brand pitch */}
        <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
          <div style={{ borderRadius: 28, background: 'linear-gradient(135deg, #1E1B4B 0%, #4C1D95 55%, #831843 100%)', padding: '40px 36px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative' }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.1)', borderRadius: 9999, padding: '4px 12px', marginBottom: 20 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'rgba(255,255,255,0.8)' }}>Quirkify Access</span>
              </div>
              <h1 style={{ fontFamily: '"Nunito", sans-serif', fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 900, lineHeight: 1.1, color: '#fff', letterSpacing: '-0.02em', marginBottom: 16 }}>
                Your quirky<br />
                <span style={{ background: 'linear-gradient(90deg,#F472B6,#C084FC)', WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>universe awaits.</span>
              </h1>
              ...
              <div style={{ display: 'grid', gap: 12 }}>
                {[
                  ...
                  return (
                    <div key={item.text} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, background: 'rgba(255,255,255,0.07)', borderRadius: 14, padding: '12px 16px', border: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.08 }}
          style={{ background: '#fff', borderRadius: 28, border: '1px solid #F3F4F6', padding: '32px 28px', boxShadow: '0 8px 40px rgba(168,85,247,0.1)' }}>

          {/* Mode toggle */}
          <div style={{ display: 'inline-flex', background: '#F3F4F6', borderRadius: 14, padding: 4, marginBottom: 28 }}>
            {(['signin', 'signup'] as const).map((item) => (
              <button key={item} onClick={() => { setMode(item); setError(null); setInfo(null); }}
                style={{ borderRadius: 10, padding: '8px 18px', fontSize: 13, fontWeight: 700, border: 'none', background: mode === item ? 'var(--gradient-primary)' : 'transparent', color: mode === item ? '#fff' : '#6B7280', cursor: 'pointer', boxShadow: mode === item ? '0 2px 8px rgba(168,85,247,0.3)' : 'none', transition: 'all 0.2s' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
            ...
            <span style={{ fontSize: 12, color: '#9CA3AF', fontWeight: 600 }}>or use email</span>
            <div style={{ flex: 1, height: 1, background: '#F3F4F6' }} />
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {mode === 'signup' && (
              <label style={{ display: 'block' }}>
                <span style={{ display: 'block', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#9CA3AF', marginBottom: 6 }}>Display name</span>
                <input value={name} onChange={(e) => setName(e.target.value)} className="input" style={{ background: '#F9FAFB', width: '100%' }} placeholder="Your name" />
              </label>
            )}

            <label style={{ display: 'block' }}>
              <span style={{ display: 'block', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#9CA3AF', marginBottom: 6 }}>Email</span>
              <div style={{ position: 'relative' }}>
                <Mail size={14} color="#9CA3AF" style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} />
                <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" required className="input" style={{ background: '#F9FAFB', paddingLeft: 40, width: '100%' }} placeholder="you@example.com" />
              </div>
            </label>

            <label style={{ display: 'block' }}>
              <span style={{ display: 'block', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#9CA3AF', marginBottom: 6 }}>Password</span>
              <div style={{ position: 'relative' }}>
                <Lock size={14} color="#9CA3AF" style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} />
                <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" required minLength={6} className="input" style={{ background: '#F9FAFB', paddingLeft: 40, width: '100%' }} placeholder="••••••••" />
              </div>
            </label>

            {info && (
              <div style={{ borderRadius: 12, background: '#F0FDF4', border: '1px solid #BBF7D0', padding: '10px 14px', fontSize: 13, color: '#15803D', display: 'flex', gap: 8, alignItems: 'flex-start' }}>
            ...
            <button disabled={busy} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: 'var(--gradient-primary)', color: '#fff', border: 'none', borderRadius: 14, padding: '14px 20px', fontSize: 14, fontWeight: 700, cursor: busy ? 'not-allowed' : 'pointer', opacity: busy ? 0.7 : 1, boxShadow: '0 4px 16px rgba(168,85,247,0.35)', transition: 'opacity 0.2s' }}>
              <ArrowRight size={16} />
