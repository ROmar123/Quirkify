        ...
        <div className="flex gap-2">
          {(['all', 'approved', 'pending', 'rejected'] as StatusFilter[]).map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={cn('filter-pill capitalize', statusFilter === s && 'active')}>
              {s}
            </button>
          ))}
        </div>
          ...
          {filtered.map(p => (
            <motion.div key={p.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button onClick={() => openEdit(p)} className="p-2 rounded-lg hover:bg-gray-50 text-gray-400 hover:text-gray-600 transition-colors" title="Edit">
        ...
        {editingProduct && (
          <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            <motion.div initial={{ y: '100%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '100%', opacity: 0 }}
              ...
              <div className="overflow-y-auto p-6 space-y-5 flex-1">
                ...
                <div>
                  ...
                  {imagePreviews.length > 0 ? (
                    <div className="flex gap-2 mt-1">
                      ...
                      {imagePreviews.length < 3 && (
                        <button onClick={() => fileInputRef.current?.click()}
                          className="w-20 h-20 rounded-2xl border-2 border-dashed border-gray-200 flex items-center justify-center text-gray-300 hover:border-gray-400 transition-colors">
                    ...
                    <div className="flex gap-2 mt-1">
                      ...
                      <button onClick={() => fileInputRef.current?.click()}
                        className="w-20 h-20 rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-300 hover:border-gray-400 transition-colors text-xs font-bold gap-1">
                        <Camera className="w-5 h-5" />Replace
                      </button>
                    </div>
                  ) : (
                    <button onClick={() => fileInputRef.current?.click()}
                      className="w-full h-24 rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center gap-2 text-gray-300 hover:border-gray-400 transition-colors">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <label className="section-label block mb-1">Store</label>
                    ...
                    <div>
                      <label className="section-label block mb-1">Auction</label>
                      <input type="number" min="0" max={editingProduct.stock ?? 1}
                    ...
                    <div>
                      <label className="section-label block mb-1">Packs</label>
                      <input type="number" min="0" max={editingProduct.stock ?? 1}
                  ...
                  <p className="text-xs text-gray-400 mt-2">Total: {(editingProduct.allocations?.store ?? 0) + (editingProduct.allocations?.auction ?? 0) + (editingProduct.allocations?.packs ?? 0)} / {editingProduct.stock ?? 1}</p>
                </div>
                  ...
                  <div className="flex gap-2">
                    {LISTING_TYPES.map(t => (
                      <button key={t} onClick={() => setField('listingType', t)}
                        className={cn('filter-pill flex-1 justify-center capitalize', editingProduct.listingType === t && 'active')}>
                        {t}
                      </button>
                    ))}
                  </div>
                ...
                <div className="sticky bottom-0 bg-white pt-4 pb-2 flex gap-3 border-t border-gray-100 -mx-6 px-6">
              ...
              <div className="hidden sm:flex p-6 border-t border-gray-100 gap-3">
                <button onClick={closeModal} className="btn-secondary flex-1 py-3 text-sm justify-center">Cancel</button>
                <button onClick={handleSave} disabled={saving} className="btn-primary flex-1 py-3 text-sm justify-center disabled:opacity-50">
                  {saving ? <><Loader2 className="w-4 h-4 animate-spin" />{savingMsg}</> : <><Save className="w-4 h-4" />{isNew ? 'Create Product' : 'Save Changes'}</>}
                </button>
