...
export default function PaymentResult({ type }: { type: 'success' | 'cancel' }) {
  ...
  const config = {
    paid: {
      ...
      primaryLabel: isWalletTopUp ? 'Back to My Vault' : 'View My Orders',
      primaryTo: isWalletTopUp ? '/collection' : '/orders',
    },
    error: {
      icon: AlertCircle,
    ...
    payment_failed: {
      ...
      sub: null,
      primaryLabel: isWalletTopUp ? 'Back to My Vault' : 'Back to Checkout',
      primaryTo: isWalletTopUp ? '/collection' : '/checkout',
    },
    cancelled: {
      icon: XCircle,
